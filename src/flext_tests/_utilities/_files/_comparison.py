...
Content, line, hash, size, and deep structured comparisons.
"""
...
class FlextTestsFilesComparisonMixin:
        ...
        *,
        ...
        try:
            if params.pattern is not None:
                ...
                result = r[bool].ok(params.pattern in text1 and params.pattern in text2)
            else:
                match params.mode:
                    case "size":
                        result = r[bool].ok(
                    ...
                    case "hash":
                        ...
                        result = r[bool].ok(hash1 == hash2)
                    case "lines":
                        result = self._compare_lines(params)
                    case _:
                        result = self._compare_content(params)
        except OSError as e:
    ...
    def _apply_key_filtering(
        ...
        if result1.success and result2.success:
            filtered1 = FlextTestsFilesCreationMixin._to_payload_mapping(result1.value)
            filtered2 = FlextTestsFilesCreationMixin._to_payload_mapping(result2.value)
    ...
    def _try_parse_both(
        ...
        if FlextTestsFilesCreationMixin._is_mapping(
            d1
        ) and FlextTestsFilesCreationMixin._is_mapping(d2):
            return (
                FlextTestsFilesCreationMixin._to_payload_mapping(d1),
                FlextTestsFilesCreationMixin._to_payload_mapping(d2),
            )
        return None
