...
class FlextTestsMakeRegistryUtilitiesMixin(FlextTestsMakeContractUtilitiesMixin):
    ...
    @staticmethod
    def make_discover(scripts_dir: Path) -> p.Result[m.Tests.MakeRegistry]:
        ...
        for verb_dir in sorted(scripts_dir.iterdir(), key=lambda item: item.name):
            ...
            for path in sorted(verb_dir.iterdir(), key=lambda item: item.name):
                if path.name == "__pycache__":
                    continue
                load_result = FlextTestsMakeRegistryUtilitiesMixin.make_load_command(
                    path,
                    verb_dir.name,
                )
                ...
                add_result = FlextTestsMakeRegistryUtilitiesMixin._make_add_command(
                    commands_by_verb,
                    aliases_by_name,
                    load_result.value,
                )
    ...
    def make_load_command(
        ...
        return FlextTestsMakeRegistryUtilitiesMixin._make_command_from_data(
            path,
            data,
            verb,
            what,
