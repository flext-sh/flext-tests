...
overlap with canonical owners. Extend the current owner when it already covers
...
A declaration that describes a population, a remainder, or a pending condition
is only true while its subject exists. When the process it governs consumes
that subject — the population gets adopted, the remainder gets resolved, the
condition gets met — the declaration stops describing anything, and its own
gate is what says so first: a count that must match reads zero because there is
nothing left to count.

Retire it at its owner, in the same change that proves the subject is gone.
Do not relax the comparison, widen the bound, or add an exemption to keep a
declaration whose referent no longer exists — that converts a precise gate into
a permissive one and hides the next real divergence behind it.

Retiring means the whole set: the configuration key, its schema type, its
validation, and every implementation only it fed. Prove first that whatever the
